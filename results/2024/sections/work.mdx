---
title: Work
position: 4
---

# Work

Many places and sectors in the world are facing economincal challenges, and the IT industry is no exception to that.
In Morocco however, developers are still a highly demanded asset, and most freshly-graduated easily find opportunities shortly after graduation, if not before.

Although we can notice a "Back to office" trend in big companies, which tends to persuade smaller companies into adopting similar decisions, remote work continues to be very popular among Moroccan developers, be it full-time remote or hybrid.

## Employment status

A reasonable portion of respondents are still students or looking for work, while most of them are already employed, either in companies via part-time, full-time or internship contracts, freelancers, or are engaging in entrepreneurial activities.

<Tabs>
  <TabItem title="2024">
    <Chart id="work-q-0" year={2024} title={false} />
  </TabItem>
  <TabItem title="2023">
    <Chart id="work-q-0" year={2023} title={false} />
  </TabItem>
  <TabItem title="2022">
    <Chart id="work-q-0" year={2022} title={false} />
  </TabItem>
  <TabItem title="2021">
    <Chart id="work-q-0" year={2021} title={false} />
  </TabItem>
  <TabItem title="2020">
    <Chart id="work-q-0" year={2020} title={false} />
  </TabItem>
</Tabs>

## Working overtime

If some respondents never work overtime, many of them find themselves doing it at different frequencies. Reasons may be tight deadlines, passion or dedication, and even sometimes company culture.

<Chart id="work-q-1" title={false} />

## Job satisfaction

Most respondents are overall satisfied with their current job, even though this satisfaction has slightly decreased compared to last year's results.

<Tabs>
  <TabItem title="2024">
    <Chart id="work-q-2" year={2024} />
  </TabItem>
  <TabItem title="2023">
    <Chart id="work-q-2" year={2023} />
  </TabItem>
  <TabItem title="2022">
    <Chart id="work-q-2" year={2022} />
  </TabItem>
  <TabItem title="2021">
    <Chart id="work-q-2" year={2021} />
  </TabItem>
  <TabItem title="2020">
    <Chart id="work-q-2" year={2020} />
  </TabItem>
</Tabs>

## Side projects

As many respondents stated that they like coding as a hobby, we find a similar proportion (70%) which is working on side projects. The motivation can be money and profit, or simply curiosity and the need to learn.

<Chart id="work-q-3" title={false} />

## Unemployment after graduation

Most respondents easily found a job right after graduation. While most companies have trouble hiring on a global scale, it seems that companies in Morocco are doing quite fine on that front.

<Chart id="work-q-4" title={false} />

## Salaries

We asked **full-time contractors** in Morocco(CDI) about their NET salary range per month in MAD is and here are the results by years of experience.

<Tabs>
  <TabItem title="1<">
    <Chart
      id="work-q-5"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["1"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="1 - 2">
    <Chart
      id="work-q-5"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="3 - 4">
    <Chart
      id="work-q-5"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["3"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="5 - 6 ">
    <Chart
      id="work-q-5"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["4"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="7 - 10 ">
    <Chart
      id="work-q-5"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["5"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="10+ ">
    <Chart
      id="work-q-5"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["6", "7"]
        }
      ]}
    />
  </TabItem>
</Tabs>

We asked **freelancer** in Morocco about their salary range per day in MAD(TJM) is and here are the results by years of experience.

<Tabs>
  <TabItem title="1<">
    <Chart
      id="work-q-6"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["1"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="1 - 2">
    <Chart
      id="work-q-6"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["2"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="3 - 4">
    <Chart
      id="work-q-6"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["3"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="5 - 6 ">
    <Chart
      id="work-q-6"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["4"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="7 - 10 ">
    <Chart
      id="work-q-6"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["5"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="10+ ">
    <Chart
      id="work-q-6"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["6", "7"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
</Tabs>

We asked **full-time contractor outside Morocco**, Your gross salary range per year in USD is and here are the results by years of experience.

<Tabs>
  <TabItem title="1<">
    <Chart
      id="work-q-7"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["1"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="1 - 2">
    <Chart
      id="work-q-7"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="3 - 4">
    <Chart
      id="work-q-7"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["3"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="5 - 6 ">
    <Chart
      id="work-q-7"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["4"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="7 - 10 ">
    <Chart
      id="work-q-7"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["5"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="10+ ">
    <Chart
      id="work-q-7"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["6", "7"]
        }
      ]}
    />
  </TabItem>
</Tabs>

We asked **freelancer outside Morocco**, Your daily gross wage in USD is and here are the results by years of experience.

<Tabs>
  <TabItem title="1<">
    <Chart
      id="work-q-8"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["1"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="1 - 2">
    <Chart
      id="work-q-8"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["2"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="3 - 4">
    <Chart
      id="work-q-8"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["3"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="5 - 6 ">
    <Chart
      id="work-q-8"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["4"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="7 - 10 ">
    <Chart
      id="work-q-8"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["5"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
  <TabItem title="10+ ">
    <Chart
      id="work-q-8"
      title={false}
      condition={[
        {
          question_id: "profile-q-5",
          values: ["6", "7"]
        },
        {
          question_id: "work-q-0",
          values: ["2"]
        }
      ]}
    />
  </TabItem>
</Tabs>

## Choosing a job offer

The results are quite comparable to previous editions. Yet it seems that respondents don't consider trainings provided by the companies as essential.
Also, the industry as well as the impact of one's work is not an important parameter in choosing a job offer.

<Tabs>
  <TabItem title="2024">
    <Chart id="work-q-9" year={2024} />
  </TabItem>
  <TabItem title="2023">
    <Chart id="work-q-9" year={2023} />
  </TabItem>
  <TabItem title="2022">
    <Chart id="work-q-9" year={2022} />
  </TabItem>
  <TabItem title="2021">
    <Chart id="work-q-9" year={2021} />
  </TabItem>

</Tabs>

## Preferred Company size

Most respondents tend to prefer smaller-sized companies. These companies naturally favour flexbility and are less rigid in processes compared to bigger ones. Smaller companies are perhaps also much less impacted by tech debt and legacy code.

<Chart id="work-q-10" title={false} />

## Agile software development methodology

More than 84% of respondents are using an agile software methodology and more than 64% of them are using scrum.

<Chart id="work-q-11" title={false} />

## Remote work

While hybrid mode is the dominant mode currently, we notice a slight decrease of full remote positions post-covid era.

<Tabs>
  <TabItem title="2024">
    <Chart id="work-q-12" year={2024} title={false} />
  </TabItem>
  <TabItem title="2023">
    <Chart id="work-q-12" year={2023} title={false} />
  </TabItem>
  <TabItem title="2022">
    <Chart id="work-q-12" year={2022} title={false} />
  </TabItem>
  <TabItem title="2021">
    <Chart id="work-q-12" year={2021} title={false} />
  </TabItem>
</Tabs>

Similar to last year, more than 90% of respondents prefer remote work, either part-time or full-time.

<Tabs>
  <TabItem title="2024">
    <Chart id="work-q-13" year={2024} title={false} />
  </TabItem>
  <TabItem title="2023">
    <Chart id="work-q-13" year={2023} title={false} />
  </TabItem>
  <TabItem title="2022">
    <Chart id="work-q-13" year={2022} title={false} />
  </TabItem>
  <TabItem title="2021">
    <Chart id="work-q-13" year={2021} title={false} />
  </TabItem>
</Tabs>
